...
public:
    ...
    bool encode_base64_and_send( int SocketFD, const unsigned char *DataToEncode, size_t DataSize )
    {
        ...
        if ( this->size > 0 )
        {
            return this->send_line( SocketFD );
        }
        return true;
    }

    bool send_line( int SocketFD )
    {
        if ( SocketFD == INVALID_SOCKET )
        {
            DBG_ONLY( cerr << "send : SocketFD == INVALID_SOCKET" );
        ...
        // on envoie une ligne, donc elle doit se terminer par "\r\n"
        bool send_crlf = true;
        if ( this->size > 1 &&
    ...
    bool receive( size_t SocketFD, int TimeOutSec )
    {
        if ( SocketFD == INVALID_SOCKET )
        {
        ...
        int res = ::select( 1, &fdread, NULL, NULL, &timeout );
        if ( !res )
        {
...
private:
    // La RFC 0821 stipulerait qu'une ligne peut faire au maximum 1000 caractères, en incluant "\r\n"
    // ce qui fait 998 caractères "libres"
    // Mais la RFC RFC 1869 semble parler de 512...
    char data[ 998 ];
    size_t size;
    bool overflow;
};
...
bool SMTPMailSender::open_smtp_connection( const char *DestMailAddress, const char *MailSubject, const char *SenderNameOrEmail, unsigned short SMTPPortNumber, int TimeOutSec )    
{
    ...
    buffer.append( "\r\n" );
    if ( !buffer.send_line( this->socket_fd ) )
    {
...
bool SMTPMailSender::send_attached_rawdata( const char *AttachmentName, const unsigned char *Data, size_t DataSize, const char *MimeType )
{
    ...
    buffer.reset( "\r\n" );
    return buffer.send_line( this->socket_fd );
}

bool SMTPMailSender::send_attached_file_from_handle( void* Win32Handle, const char *AttachmentName, const char *MimeType )
{
    HANDLE hFile = Win32Handle;
    if ( !hFile )
    {
        return false;
    }
...
bool SMTPMailSender::send_attached_file( const char *FileName, const char *FullPathFileName, const char *MimeType )
{
    ...
    bool res = this->send_attached_file_from_handle( hFile, FileName, MimeType );
